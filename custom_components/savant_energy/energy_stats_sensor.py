"""Energy Statistics Sensor for Savant Energy."""
...
class EnergyStatsSensor(RestoreEntity, SensorEntity):
    """Energy statistics sensor with daily, monthly, yearly and lifetime tracking."""
    ...
    def _async_reset_daily(self, _now: Optional[datetime] = None) -> None:
        ...
        _LOGGER.debug("%s: Reset daily energy stats", self.entity_id)
        self.async_write_ha_state()
    ...
    def _async_reset_monthly(self, _now: Optional[datetime] = None) -> None:
        ...
        _LOGGER.debug("%s: Reset monthly energy stats", self.entity_id)
        self.async_write_ha_state()
    ...
    def extra_state_attributes(self) -> Dict[str, Any]:
        ...
        return {
            ATTR_DAILY_USAGE: float(self._energy[PERIOD_DAILY]),
            ATTR_MONTHLY_USAGE: float(self._energy[PERIOD_MONTHLY]),
            ATTR_YEARLY_USAGE: float(self._energy[PERIOD_YEARLY]),
